...
function runTestsV178R4(){
  ...
  eq('Revision r4: Schema 17',17,SCHEMA_VERSION);eq('Revision r4: Regelversion 7',7,RULES_VERSION);eq('Revision r4: 405 Kräfte im erweiterten Katalog',405,POWER_ENTRIES_V176.length);
  ...
  const coverage=exhaustiveSynergyPowerCoverageV178R4();eq('Volltest: 32 Schul-Counter-Kombinationen',32,coverage.assignmentCases);eq('Volltest: 480 einzelne Lern- und Filterfälle',480,coverage.individualCases);eq('Volltest: alle 210 Zauber erfasst',210,coverage.kindCounts.spell);eq('Volltest: alle 75 Wunder erfasst',75,coverage.kindCounts.miracle);eq('Volltest: alle 120 Flüche erfasst',120,coverage.kindCounts.curse);eq('Volltest: alle 405 IDs in Synergieauswahlen sichtbar',405,coverage.visibleIds.length);eq('Volltest: kein Lern-, Filter-, Auswahl- oder Statusfehler',0,coverage.failures.length);
