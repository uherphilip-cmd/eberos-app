...
function runTestsV178R4(){
  ...
  eq('Revision r4: Schema 17',17,SCHEMA_VERSION);eq('Revision r4: Regelversion 7',7,RULES_VERSION);eq('Revision r4: 450 Kräfte im erweiterten Katalog',450,POWER_ENTRIES_V176.length);
  ...
  const coverage=exhaustiveSynergyPowerCoverageV178R4();eq('Volltest: 35 Schul-Counter-Kombinationen',35,coverage.assignmentCases);eq('Volltest: 525 einzelne Lern- und Filterfälle',525,coverage.individualCases);eq('Volltest: alle 255 Zauber erfasst',255,coverage.kindCounts.spell);eq('Volltest: alle 75 Wunder erfasst',75,coverage.kindCounts.miracle);eq('Volltest: alle 120 Flüche erfasst',120,coverage.kindCounts.curse);eq('Volltest: alle 450 IDs in Synergieauswahlen sichtbar',450,coverage.visibleIds.length);eq('Volltest: kein Lern-, Filter-, Auswahl- oder Statusfehler',0,coverage.failures.length);
