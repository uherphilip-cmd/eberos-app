...
function formatSynergyRuleTextV1721R2(value,owner,path){return formatRuleTextV176(value,synergyRollSummaryV1721R2(owner,path).s)}
function synergyValueBadgesV1721R2(owner,path){
  ...
  current.append(el('span',{class:'power-badge-v176',text:`S = ${roll.s}`}),el('span',{class:'power-badge-v176',text:`W = ${roll.die}`}),el('span',{class:'power-badge-v176',text:`B = ${roll.b}`}));return current;
}
...
combatTechniqueInfoContentV176=function(entry,owner){
  ...
  const path=entry.synergyPath,box=el('div',{class:'power-detail-v176'}),grid=el('div',{class:'power-detail-grid-v176'}),current=value=>formatSynergyRuleTextV1721R2(value,owner,path);
  grid.append(
    combatTechniqueDetailSectionV176('Aktivierung',current(entry.activation)),
    combatTechniqueDetailSectionV176('Wirkung',current(entry.effect)),
    combatTechniqueDetailSectionV176('Skalierung','Magischer Zusatz: aktuelle Werte aus der niedrigeren wirksamen Elternstufe. Körperlicher Teil: wirksame Kampfstufe.'),
    combatTechniqueDetailSectionV176('Probe / Gegenwehr',current(entry.check)),
    combatTechniqueDetailSectionV176('Regeln & Grenzen',current(entry.limits))
  );
  box.append(synergyValueBadgesV1721R2(owner,path),grid);return box;
};
combatTechniqueShortTextV176=function(entry,owner){return entry?.synergyHybridV1721R2?formatSynergyRuleTextV1721R2(entry.effect,owner,entry.synergyPath):combatTechniqueShortBeforeV1721R2(entry,owner)};
...
function renderSynergyPowerV1721R2(owner,path,power){
  ...
  detail.append(synergyValueBadgesV1721R2(owner,path),el('p',{text:formatSynergyRuleTextV1721R2(power.text,owner,path)}));if(power.checks?.length)detail.append(el('p',{class:'muted',text:'Prüfhinweise: '+formatSynergyRuleTextV1721R2(power.checks.join(' · '),owner,path)}));if(path.type==='Schule'&&status==='learned')detail.append(synergySchoolActionV1721R2(owner,path,power));
...
function renderSynergyPathV1721R2(owner,path){
  ...
  const body=el('div',{class:'catalog-synergy-path-body-v1721r2'});body.append(el('p',{text:formatSynergyRuleTextV1721R2(path.concept,owner,path)}),el('p',{class:'muted',text:`Grundwerte: ${path.attributes.join(' + ')} · Stabile ID: ${path.id}`}));if(path.context)body.append(el('p',{text:formatSynergyRuleTextV1721R2(path.context,owner,path)}));
...
renderPrintableCardR15=function(card,owner,options,isCharacter){
  ...
  for(const entry of HYBRID_ENTRIES_V1721R2.filter(value=>owner.skills?.[value.skillId]?.learnedTechniqueIds?.includes(value.id))){
    ...
    const paragraphs=article.querySelectorAll('p'),current=value=>formatSynergyRuleTextV1721R2(value,owner,entry.synergyPath);
    if(paragraphs[0])paragraphs[0].textContent=`Aktivierung: ${current(entry.activation)}`;
    if(paragraphs[1])paragraphs[1].textContent=`Wirkung: ${current(entry.effect)}`;
    if(paragraphs[2])paragraphs[2].textContent=`Probe/Gegenwehr: ${current(entry.check)}`;
    if(paragraphs[3])paragraphs[3].textContent=`Grenzen: ${current(entry.limits)}`;
  }
  return node;
};
