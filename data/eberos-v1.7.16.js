...
const V1716_VERSION='1.7.16',V1716_REVISION='r1',V1716_SCHEMA=24,V1716_RULES=9,V1716_CATALOG_VERSION='1.2.0';
const V1716_SKILLS=[
  {
...
function runTestsV1716(){
  ...
  eq('Version 1.7.16',V1716_VERSION,APP_VERSION);eq('Schema 24',V1716_SCHEMA,SCHEMA_VERSION);eq('Regelstand 9',V1716_RULES,RULES_VERSION);eq('Revision v1.7.16 r1','r1',V1716_REVISION);eq('Katalogversion 1.2.0',V1716_CATALOG_VERSION,POWER_DB_V176.meta?.catalogVersion);
  eq('85 Regelfähigkeiten',85,SKILLS.length);eq('85 eindeutige Regelfähigkeits-IDs',85,new Set(SKILLS.map(function(skill){return skill.id})).size);eq('Grundwertverteilung v1.7.16','ST:14|KS:16|GS:14|RF:14|FF:18|WL:17|IT:23|IN:19|WN:18|CR:17',ATTRS.map(function(attribute){return attribute[0]+':'+attributeDistributionV1716()[attribute[0]]}).join('|'));
  ...
  eq('435 Kräfte',435,POWER_ENTRIES_V176.length);eq('29 Kraftschulen',29,POWER_SCHOOLS_V176.length);eq('435 eindeutige Kraft-IDs',435,new Set(POWER_ENTRIES_V176.map(function(entry){return entry.id})).size);eq('240 Mana-Zauber',240,POWER_ENTRIES_V176.filter(function(entry){return entry.pathId==='M'}).length);eq('30 neue Zauber',30,POWER_ENTRIES_V176.filter(function(entry){return V1716_SCHOOL_IDS.has(entry.schoolId)}).length);eq('Neue Schulen vollständig',true,[...V1716_SCHOOL_IDS].every(function(id){return POWER_ENTRIES_V176.filter(function(entry){return entry.schoolId===id}).map(function(entry){return entry.code}).join('|')===Array.from({length:15},function(_,index){return'Z'+(index+1)}).join('|')}));
