...
function effectiveSkillLevelV1721R2(owner,skillId){
  ...
  return Math.max(0,Math.min(25,purchased+(skillModifiers(owner)?.[skillId]||0)));
}
...
function formatSynergyRuleTextV1721R2(value,owner,path){
  const readableVariables=String(value||'').replace(/\bS\s*\/\s*B\s*\/\s*W\b/g,'Stufe S, Bonus B und Würfel W').replace(/\bS\s*\/\s*B\b/g,'Stufe S und Bonus B').replace(/\bB\s*\/\s*W\b/g,'Bonus B und Würfel W');
  return formatRuleTextV176(readableVariables,synergyRollSummaryV1721R2(owner,path).s);
}
const SYNERGY_COUNTER_NAMES_V1721R2={A:'Ausdauer (A)',M:'Mana (M)',GB:'Glaube (GB)',FS:'Finsternis (FS)',L:'Leben (LP)'};
const SYNERGY_PLAYER_SECTION_ORDER_V1721R2=['Einsatz','Ziel','Wirkung','Probe & Gegenwehr','Dauer & Ende','Bei Fehlschlag','Grenzen'];
function synergyPlayerSectionLabelV1721R2(source){
  const label=String(source||'').trim();
  if(/Balance|Buchung\/Abgrenzung|Elternvergleich|Ausgangsschulen|Ausgangsfähigkeiten|Motiv/i.test(label))return'';
  if(/Typ|Kosten|Einsatz/i.test(label))return'Einsatz';
  if(/Ziel|Voraussetzung/i.test(label))return'Ziel';
  if(/Wirkung|Körper/i.test(label))return'Wirkung';
  if(/Probe|Gegenwehr|Vergleichsentscheidung/i.test(label))return'Probe & Gegenwehr';
  if(/Fehlschlag|Unterbrechung/i.test(label))return'Bei Fehlschlag';
  if(/Grenz/i.test(label))return'Grenzen';
  if(/Dauer|Ende|Gegenspiel/i.test(label))return'Dauer & Ende';
  if(/^Buchung$/i.test(label)||/Abgrenzung/i.test(label))return'';
  return label||'Wirkung';
}
function cleanSynergyPlayerTextV1721R2(value){
  let text=String(value||'').replace(/(?:&#x20;|&#32;|&nbsp;)/gi,' ').replace(/\[([^\]]+)\]\([^)]+\)/g,'$1').replace(/\*\*|`|\*/g,'').replace(/\bQuellwirkstufe\b/gi,'wirksame Stufe der fremden Kraft').replace(/\bElternstufe(?:n)?\b/gi,'Stufe der beteiligten Fähigkeiten').replace(/\b([A-ZÄÖÜ]{1,4}\d+)-artig(?:e|en|er|es)?\b/g,'entsprechend').replace(/\bWirkungstext(?:es)?\b/gi,'Beschreibung').replace(/\blegalen körperlichen Vorgang\b/gi,'gültige körperliche Aktion').replace(/\bKrafttext(?:es)?\b/gi,'Beschreibung der Kraft').replace(/\s+/g,' ').trim();
  text=text.replace(/Das durch Angriff, Block, Griff oder Bewegung regelgerecht erreichbare Ziel, Feld oder Objekt aus der Beschreibung\./gi,'Ein für die beschriebene körperliche Aktion erreichbares Ziel, Feld oder Objekt.').replace(/Es gilt die normale Reichweite des körperlichen Vorgangs; eine dort genannte engere Linie, Berührung, Deckung oder Ankerbedingung geht vor\./gi,'Es gelten die normale Reichweite und alle engeren Bedingungen aus der Beschreibung.').replace(/Ohne gültige körperliche Aktion beginnt die Technik nicht\./gi,'Ohne diese körperliche Aktion kann die Technik nicht eingesetzt werden.').replace(/Ein Synergiewurf mit ([^.;]+); der Würfel folgt der wirksamen Kampfstufe\./gi,'Probe: $1 mit dem Würfel der wirksamen Kampfstufe.').replace(/Derselbe Wurf entscheidet den körperlichen Vorgang und den Zusatz\./gi,'Derselbe Wurf entscheidet die körperliche Aktion und den magischen Zusatz.').replace(/Das Ziel verwendet die normale körperliche Abwehr; ein widerstrebender magischer Zusatz nutzt innerhalb dieses Vergleichs die passende geistige, seelische oder magische Gegenwehr\./gi,'Das Ziel nutzt seine normale körperliche Abwehr; gegen den magischen Zusatz gilt die passende geistige, seelische oder magische Gegenwehr.').replace(/Gegen fremde Wirkungen gilt zusätzlich wirksame Stufe der fremden Kraft\s*≤\s*(\d+)\./gi,'Fremde Kräfte lassen sich nur beeinflussen, wenn ihre wirksame Stufe höchstens $1 beträgt.');
  text=text.replace(/;\s*(?:speichern|protokollieren|notieren)\s*:[^.!?]*(?=[.!?]|$)/gi,'').replace(/\bmin\((\d+),\s*([^)]+)\)/gi,'höchstens $1, begrenzt durch $2');
  const sentences=text.split(/(?<=[.!?])\s+/).filter(sentence=>{
    const compact=sentence.trim();
    return compact&&!/^(?:Anders als|Gegenüber|Abgrenzung|Balancevorbehalt|Pfadabgrenzung|Der Builder|Im Builder|Für den Builder|Die Zahlung erfolgt vollständig|Eine ausdrücklich spätere Folgezahlung|Der konkrete Wirkungstext|Gleiche eigene Wirkungen stapeln|Der Effekt erzeugt keine zusätzliche Aktion)/i.test(compact)&&!/\b(?:Stabile ID|Inhalts-ID|Katalogprofil|vor Freigabe|ursprünglichen .*Fassungen|in diesem Dokument|skill_[a-z0-9_]+|speichern|protokollieren|notieren|fortgeschrieben)\b/i.test(compact);
  });
  text=sentences.join(' ').replace(/\s+([,.;:!?])/g,'$1').trim();
  return text.replace(/\b(\d+)\s+GB\b/g,'$1 Glaube (GB)').replace(/\b(\d+)\s+FS\b/g,'$1 Finsternis (FS)').replace(/\b(\d+)\s+LP\b/g,'$1 Leben (LP)').replace(/\b(\d+)\s+M\b/g,'$1 Mana (M)').replace(/\b(\d+)\s+A\b/g,'$1 Ausdauer (A)');
}
function trimSynergyActivationV1721R2(value){
  let text=String(value||'').trim().replace(/\*\*/g,'').replace(/^(?:Zauber|Wunder|Fluch|Hybridtechnik)(?:[-/][A-Za-zÄÖÜäöüß]+)*\s*[,;]\s*/i,'');const stop=text.search(/[,.;]/),lead=stop>=0?text.slice(0,stop):text;if(/\b\d+\s*(?:eigene(?:r|n)?\s+)?(?:A|M|GB|FS|LP)\b/i.test(lead))text=stop>=0?text.slice(stop+1).trim():'';
  return text;
}
function synergyPlayerSectionsV1721R2(value,owner,path){
  const text=formatSynergyRuleTextV1721R2(value,owner,path),matches=[...text.matchAll(/\*\*([^*]+):\*\*/g)],collected=new Map();
  const add=(rawLabel,rawValue)=>{const label=synergyPlayerSectionLabelV1721R2(rawLabel);if(!label)return;let content=label==='Einsatz'?trimSynergyActivationV1721R2(rawValue):rawValue;if(label==='Dauer & Ende'&&/^Ende\/Buchung/i.test(rawLabel)){const semicolon=content.indexOf(';');content=semicolon>=0?content.slice(semicolon+1):content.replace(/^[^.!?]*[.!?]\s*/, '')}content=cleanSynergyPlayerTextV1721R2(content);if(!content)return;collected.set(label,[collected.get(label),content].filter(Boolean).join(' '))};
  const prefix=text.slice(0,matches[0]?.index??text.length).trim();
  if(prefix){const lead=prefix.match(/^\*\*([^*]+)\*\*\s*([\s\S]*)$/);if(lead&&/\b\d+\s*(?:A|M|GB|FS|LP)\b/.test(lead[1])){add('Einsatz',lead[1]);add('Wirkung',lead[2])}else add('Wirkung',prefix)}
  matches.forEach((match,index)=>add(match[1],text.slice((match.index||0)+match[0].length,matches[index+1]?.index??text.length)));
  if(!collected.size)add('Wirkung',text);
  return[...collected].map(([label,content])=>({label,content})).sort((a,b)=>{const ai=SYNERGY_PLAYER_SECTION_ORDER_V1721R2.indexOf(a.label),bi=SYNERGY_PLAYER_SECTION_ORDER_V1721R2.indexOf(b.label);return(ai<0?99:ai)-(bi<0?99:bi)});
}
function synergyPlayerCostV1721R2(power){
  const phrase=cleanSynergyPlayerTextV1721R2(costPhraseV1721R2(power.cost));if(phrase)return phrase;const parts=costPartsV1721R2(power.cost);return parts.map(part=>`${part.amount} ${SYNERGY_COUNTER_NAMES_V1721R2[part.counterId]||part.counterId}`).join(' + ')||'besondere Kosten im Text';
}
function synergyPlayerRequirementV1721R2(path,power){const threshold=thresholdValuesV1721R2(power.threshold);return path.type==='Kampf'?`Kampffähigkeit ${threshold.first} + Schule ${threshold.second}`:`beide Schulen auf Stufe ${threshold.first}`}
function synergyPlayerSummaryV1721R2(owner,path,power){
  const sections=synergyPlayerSectionsV1721R2(power.text,owner,path),source=sections.find(section=>section.label==='Wirkung')||sections.find(section=>section.label==='Ziel')||sections.find(section=>section.label!=='Einsatz')||sections[0],text=source?.content||'';
  const first=text.split(/(?<=[.!?])\s+/)[0]||text;if(first.length<=190)return first;const clipped=first.slice(0,187).replace(/\s+\S*$/,'');return clipped+' …';
}
function synergyValueBadgesV1721R2(owner,path){
  const roll=synergyRollSummaryV1721R2(owner,path),current=el('div',{class:'power-current-v176 catalog-synergy-values-v1721r2'});
  current.append(el('span',{class:'power-badge-v176',text:`Synergiestufe ${roll.s}`}),el('span',{class:'power-badge-v176',text:`Würfel ${roll.die}`}),el('span',{class:'power-badge-v176',text:`Bonus +${roll.b}`}));return current;
}
function synergyPlayerInfoContentV1721R2(owner,path,power){
  const box=el('div',{class:'power-detail-v176 synergy-player-info-v1721r2'}),grid=el('div',{class:'synergy-player-sections-v1721r2'});for(const section of synergyPlayerSectionsV1721R2(power.text,owner,path))grid.append(detailSectionV176(section.label,section.content));box.append(synergyValueBadgesV1721R2(owner,path),grid);return box;
}
function synergyPlayerPathConceptV1721R2(path){
  const clauses=String(path.concept||'').split(/;\s*/).filter(clause=>!/\b(?:anders als|gegenüber|Katalog|Profil|Entwurf|Z\d+)\b/i.test(clause));return cleanSynergyPlayerTextV1721R2(clauses[0]||path.concept||'');
}
...
function synergySchoolActionV1721R2(owner,path,power){
  ...
  return actionDisclosureV1712R2(`Kraft einsetzen · ${costPhraseV1721R2(power.cost)}`,`${power.name}: Synergieeinsatz vorbereiten`,host=>{
    ...
    host.append(el('strong',{text:`${power.name} – Einsatz bestätigen`}),el('p',{text:`Probe: ${roll.attributeText} + ${roll.die}. Synergiestufe ${roll.s} · Bonus +${roll.b}. Würfle die Probe am Spieltisch.`}),el('p',{class:'muted',text:'Die Kosten werden mit der Bestätigung abgezogen und bleiben auch bei einem Fehlschlag verbraucht.'}));appendPaymentSummaryV1712R2(host,plan,components);
...
combatTechniqueActionV1712R2=function(entry,owner){
  ...
  return actionDisclosureV1712R2(`Hybridtechnik einsetzen · ${costPhraseV1721R2(power.cost)}`,`${entry.name}: Hybridtechnik vorbereiten`,host=>{
    ...
    host.append(el('strong',{text:`${entry.name} – Einsatz bestätigen`}),el('p',{text:`Probe: ${roll.attributeText} + Würfel der wirksamen Kampfstufe. Magischer Zusatz: Synergiestufe ${roll.s}, ${roll.die}, Bonus +${roll.b}.`}),el('p',{class:'muted',text:`${payment.first?'Erste Technik dieser Runde.':'Weitere Technik derselben Runde.'} Die angezeigte Ausdauer ist der gesamte Preis für diesen Einsatz.`}));appendPaymentSummaryV1712R2(host,plan,components);
    if(preparedBinding)host.append(el('p',{class:'notice',text:'Voraussetzung: Eine passende Runenbindung wurde bereits vorbereitet. Hier wird nur die Ausdauer abgezogen; Auswahl und Verbrauch der Rune folgen ihrer Beschreibung.'}));
...
const combatTechniqueInfoBeforeV1721R2=combatTechniqueInfoContentV176,combatTechniqueShortBeforeV1721R2=combatTechniqueShortTextV176;
combatTechniqueInfoContentV176=function(entry,owner){
  if(!entry?.synergyHybridV1721R2)return combatTechniqueInfoBeforeV1721R2(entry,owner);
  const power=SYNERGY_POWER_BY_ID_V1721R2.get(entry.id);return synergyPlayerInfoContentV1721R2(owner,entry.synergyPath,power);
};
combatTechniqueShortTextV176=function(entry,owner){if(!entry?.synergyHybridV1721R2)return combatTechniqueShortBeforeV1721R2(entry,owner);const power=SYNERGY_POWER_BY_ID_V1721R2.get(entry.id);return synergyPlayerSummaryV1721R2(owner,entry.synergyPath,power)};
...
function renderSynergyPowerV1721R2(owner,path,power){
  ...
  head.append(el('div',{},[el('strong',{text:`${power.slot}. ${power.name}`}),el('small',{text:`Ab ${synergyPlayerRequirementV1721R2(path,power)} · ${synergyPlayerCostV1721R2(power)}`}),el('span',{class:'catalog-synergy-preview-v1721r2',text:synergyPlayerSummaryV1721R2(owner,path,power)})]),el('span',{class:`catalog-synergy-status-v1721r2 status-${status}`,text:statusLabelV1721R2(status)}));
  ...
  detail.append(synergyPlayerInfoContentV1721R2(owner,path,power));if(power.checks?.length)detail.append(detailSectionV176('Zusätzlicher Hinweis',cleanSynergyPlayerTextV1721R2(formatSynergyRuleTextV1721R2(power.checks.join(' · '),owner,path))));if(path.type==='Schule'&&status==='learned')detail.append(synergySchoolActionV1721R2(owner,path,power));
...
function renderSynergyPathV1721R2(owner,path){
  ...
  summary.append(el('div',{},[el('strong',{text:path.name}),el('small',{text:`${path.type==='Schule'?'Schulsynergie':'Hybridpfad'} · ${pathParentLabelV1721R2(owner,path)}`})]),el('span',{class:`catalog-synergy-status-v1721r2 path-${discovery}`,text:status}));node.append(summary);
  const body=el('div',{class:'catalog-synergy-path-body-v1721r2'});body.append(el('p',{class:'catalog-synergy-concept-v1721r2',text:synergyPlayerPathConceptV1721R2(path)}),el('p',{class:'muted',text:`Probe mit ${path.attributes.join(' + ')}`}));if(path.context){const rules=cleanSynergyPlayerTextV1721R2(formatSynergyRuleTextV1721R2(path.context,owner,path));if(rules){const more=el('details',{class:'catalog-synergy-path-rules-v1721r2'},[el('summary',{text:'Gemeinsame Regeln dieses Pfads'}),el('p',{text:rules})]);body.append(more)}}
...
function renderCatalogSynergiesV1721R2(owner){
  ...
  head.append(el('div',{},[el('h3',{text:'Synergien'}),el('p',{class:'muted',text:'Kombinationen aus zwei Fähigkeiten. Werte und Würfel passen sich automatisch an deine aktuellen Stufen an.'})]),el('div',{class:'catalog-synergy-pool-v1721r2'},[el('strong',{text:`Schulzauber ${active}/${capacity}`}),el('span',{text:`${learned} gelernt`})]));section.append(head);
  section.append(el('details',{class:'catalog-synergy-guide-v1721r2'},[el('summary',{text:'So liest und benutzt du Synergien'}),el('div',{},[el('p',{text:'Auf jeder Kraft stehen bereits deine aktuelle Synergiestufe, dein Würfel und dein Bonus. Die Beschreibung verwendet diese fertigen Werte.'}),el('ul',{},[el('li',{text:'Ohne den Hinweis „Ritual“ oder „Reaktion“ kostet eine Schulkraft eine Aktion.'}),el('li',{text:'Du bezahlst die genannten Kosten beim Beginn. Scheitert die Probe, bleiben sie verbraucht.'}),el('li',{text:'Du würfelst einmal mit den beim Pfad genannten Attributen und dem angezeigten Würfel. Ein betroffenes Ziel nutzt die passende körperliche, geistige oder seelische Abwehr.'}),el('li',{text:'Dauern richten sich nach deiner Synergiestufe. Begrenzte Mengen oder zusätzliche Ziele richten sich nach deinem Bonus.'}),el('li',{text:'Wenn die Kraft keine engere Entfernung nennt, muss ihr Ziel innerhalb deiner angezeigten Synergiestufe in Feldern liegen.'})])])]));
  ...
  type.append(el('option',{value:'all',text:'Schulzauber und Hybridtechniken'}),el('option',{value:'Schule',text:'Schulzauber'}),el('option',{value:'Kampf',text:'Hybridtechniken'}));
...
const renderPrintableBeforeV1721R2=renderPrintableCardR15;
renderPrintableCardR15=function(card,owner,options,isCharacter){
  const node=renderPrintableBeforeV1721R2(card,owner,options,isCharacter);if(card.type!=='skills')return node;
  for(const entry of HYBRID_ENTRIES_V1721R2.filter(value=>owner.skills?.[value.skillId]?.learnedTechniqueIds?.includes(value.id))){
    const article=[...node.querySelectorAll('.print-skill-description')].find(value=>value.querySelector('h4')?.textContent.startsWith(`${entry.code} · ${entry.name} ·`));if(!article)continue;
    const power=SYNERGY_POWER_BY_ID_V1721R2.get(entry.id),heading=article.querySelector('h4'),roll=synergyRollSummaryV1721R2(owner,entry.synergyPath);heading.textContent=`${entry.name} · Hybridtechnik`;article.querySelectorAll('p').forEach(paragraph=>paragraph.remove());article.append(el('p',{text:`Ab ${synergyPlayerRequirementV1721R2(entry.synergyPath,power)} · ${synergyPlayerCostV1721R2(power)} · Synergiestufe ${roll.s}, ${roll.die}, Bonus +${roll.b}`}));for(const section of synergyPlayerSectionsV1721R2(power.text,owner,entry.synergyPath))article.append(el('p',{text:`${section.label}: ${section.content}`}));
  }
  return node;
};
...
.catalog-synergies-v1721r2{margin-top:1rem;border-top:2px solid var(--accent-2);padding-top:.8rem}.catalog-synergy-heading-v1721r2{display:flex;justify-content:space-between;gap:.8rem;align-items:flex-start}.catalog-synergy-heading-v1721r2 h3{margin:.1rem 0}.catalog-synergy-pool-v1721r2{display:flex;flex-direction:column;gap:.1rem;padding:.45rem .65rem;border:1px solid var(--accent-2);border-radius:8px;background:var(--panel-alt);white-space:nowrap}.catalog-synergy-guide-v1721r2{margin:.65rem 0;padding:.55rem .65rem;border:1px solid var(--accent-2);border-radius:8px;background:var(--panel-alt)}.catalog-synergy-guide-v1721r2>summary{cursor:pointer;font-weight:700}.catalog-synergy-guide-v1721r2 p{margin:.55rem 0}.catalog-synergy-guide-v1721r2 ul{margin:.35rem 0 .1rem;padding-left:1.25rem}.catalog-synergy-guide-v1721r2 li+li{margin-top:.28rem}.catalog-synergy-filters-v1721r2{display:grid;grid-template-columns:minmax(12rem,1fr) auto auto}.catalog-synergy-list-v1721r2{display:grid;gap:.55rem}.catalog-synergy-path-v1721r2{border:1px solid var(--border);border-radius:9px;background:var(--panel-alt)}.catalog-synergy-path-v1721r2>summary{display:flex;justify-content:space-between;gap:.7rem;align-items:center;padding:.65rem;cursor:pointer}.catalog-synergy-path-v1721r2>summary div{display:grid;gap:.1rem}.catalog-synergy-path-body-v1721r2{display:grid;gap:.5rem;padding:0 .65rem .7rem}.catalog-synergy-concept-v1721r2{margin:.15rem 0 0}.catalog-synergy-path-rules-v1721r2{padding:.4rem .55rem;border-left:3px solid var(--accent-2);background:var(--panel-bg)}.catalog-synergy-path-rules-v1721r2>summary{cursor:pointer;font-weight:600}.catalog-synergy-path-rules-v1721r2 p{margin:.45rem 0 0}.catalog-synergy-power-v1721r2{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.5rem;padding:.65rem;border:1px solid var(--border);border-radius:8px;background:var(--panel-bg)}.catalog-synergy-power-head-v1721r2{display:flex;justify-content:space-between;gap:.5rem}.catalog-synergy-power-head-v1721r2>div{display:grid;gap:.18rem}.catalog-synergy-preview-v1721r2{margin-top:.15rem;line-height:1.42}.catalog-synergy-status-v1721r2{align-self:start;padding:.14rem .4rem;border:1px solid var(--border);border-radius:999px;font-size:.72rem;white-space:nowrap}.catalog-synergy-status-v1721r2.status-learned,.catalog-synergy-status-v1721r2.path-open{border-color:var(--ok);color:var(--ok)}.catalog-synergy-status-v1721r2.status-resting,.catalog-synergy-status-v1721r2.status-prerequisite{border-color:var(--warn);color:var(--warn)}.catalog-synergy-actions-v1721r2{display:flex;gap:.35rem;flex-wrap:wrap;justify-content:flex-end}.catalog-synergy-detail-v1721r2{grid-column:1/-1}.synergy-player-info-v1721r2{display:grid;gap:.65rem}.synergy-player-sections-v1721r2{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.55rem}.synergy-player-sections-v1721r2 .power-detail-section-v176{padding:.6rem;border:1px solid var(--border);border-radius:7px;background:var(--panel-alt)}.synergy-player-sections-v1721r2 .power-detail-section-v176 h4{margin:0 0 .3rem;color:var(--accent-2)}.synergy-player-sections-v1721r2 .power-detail-section-v176 p{margin:0;white-space:normal;line-height:1.45}.catalog-synergy-power-v1721r2.status-locked{opacity:.75}@media(max-width:760px){.catalog-synergy-heading-v1721r2{display:grid}.catalog-synergy-filters-v1721r2,.synergy-player-sections-v1721r2{grid-template-columns:1fr}.catalog-synergy-power-v1721r2{grid-template-columns:1fr}.catalog-synergy-actions-v1721r2{justify-content:flex-start}.catalog-synergy-power-head-v1721r2{align-items:flex-start}}@media print{.catalog-synergy-filters-v1721r2,.catalog-synergy-actions-v1721r2,.catalog-synergy-detail-v1721r2>.learned-action-v1712r2{display:none!important}.catalog-synergy-path-v1721r2:not([open]){display:none}.catalog-synergy-power-v1721r2.status-locked,.catalog-synergy-power-v1721r2.status-available{display:none}.catalog-synergy-detail-v1721r2[hidden]{display:block!important}.synergy-player-sections-v1721r2{grid-template-columns:1fr 1fr}}
...
function runTestsV1721R2(){
  ...
  eq('Hybridanzeige skaliert mit niedrigerer Elternstufe',true,combatTechniqueInfoContentV176(hybrid.entry,hybridOwner).textContent.includes('Synergiestufe 5'));hybridOwner.counters.A={max:10,current:10};hybridOwner.counters.M={max:10,current:10};hybridOwner.counters.GB={max:10,current:10};hybridOwner.counters.FS={max:10,current:10};const hybridCosts=hybridPaymentComponentsV1721R2(hybridOwner,hybrid.entry),hybridBefore=Object.fromEntries(hybridCosts.components.map(part=>[part.counterId,hybridOwner.counters[part.counterId].current]));useTechniqueV1721(hybridOwner,hybridPower.id,{persist:false,notify:false,debounce:false});eq('Hybrid bucht A und übernatürliche Kosten atomar',true,hybridCosts.components.every(part=>hybridOwner.counters[part.counterId].current===hybridBefore[part.counterId]-part.amount));
  const readableOwner=newCharacter();for(const skill of SKILLS)readableOwner.skills[skill.id].level=25;const readableSections=SYNERGY_POWERS_V1721R2.flatMap(power=>synergyPlayerSectionsV1721R2(power.text,readableOwner,SYNERGY_PATH_BY_ID_V1721R2.get(power.pathId))),conciseRules=SYNERGY_POWERS_V1721R2.filter(power=>power.revision==='new_profile'&&!power.text.includes('**Typ/Kosten/Einsatz:**'));eq('Alle Synergien besitzen eine Spielerbeschreibung',345,SYNERGY_POWERS_V1721R2.filter(power=>synergyPlayerSectionsV1721R2(power.text,readableOwner,SYNERGY_PATH_BY_ID_V1721R2.get(power.pathId)).length>0).length);eq('Spielerbeschreibungen blenden Redaktion und Buchung aus',true,readableSections.every(section=>!/(?:Buchung|Balancevorbehalt|Stabile ID|Inhalts-ID|skill_[a-z0-9_]+|\*\*|`)/i.test(`${section.label} ${section.content}`)));eq('240 Kurzkonzepte besitzen genau eine klare Wirkungsbeschreibung',240,conciseRules.filter(power=>synergyPlayerSectionsV1721R2(power.text,readableOwner,SYNERGY_PATH_BY_ID_V1721R2.get(power.pathId)).length===1).length);eq('Kurze Spielertexte enthalten keine Redaktionsvorlagen oder unerklärte Platzhalter',true,conciseRules.every(power=>!/(?:Das im Wirkungstext|Einmalig festhalten|Inhalts-ID|Pfadabgrenzung|konkrete Zuordnung|noch prüfen|Quellwirkstufe|Wirkwert|\bAnker\b|\bTräger\b|min\()/i.test(power.text)));
