...
function runTestsV1721R2(){
  ...
  const readableOwner=newCharacter();for(const skill of SKILLS)readableOwner.skills[skill.id].level=25;const readableSections=SYNERGY_POWERS_V1721R2.flatMap(power=>synergyPlayerSectionsV1721R2(power.text,readableOwner,SYNERGY_PATH_BY_ID_V1721R2.get(power.pathId)));eq('Alle Synergien besitzen eine Spielerbeschreibung',345,SYNERGY_POWERS_V1721R2.filter(power=>synergyPlayerSectionsV1721R2(power.text,readableOwner,SYNERGY_PATH_BY_ID_V1721R2.get(power.pathId)).length>0).length);eq('Spielerbeschreibungen blenden Redaktion und Buchung aus',true,readableSections.every(section=>!/(?:Buchung|Balancevorbehalt|Stabile ID|Inhalts-ID|skill_[a-z0-9_]+|\*\*|`)/i.test(`${section.label} ${section.content}`)));
