...
function runTestsV1715(){
  ...
  eq('86 aktive Regelfähigkeiten',86,SKILLS.length);eq('86 eindeutige IDs',86,new Set(SKILLS.map(function(skill){return skill.id})).size);eq('86 eindeutige Namen',86,new Set(SKILLS.map(function(skill){return skill.name})).size);eq('Bannung & Schutzriten nicht aktiv',false,SKILLS.some(function(skill){return skill.id===V1715_RETIRED_SKILL.id||skill.name===V1715_RETIRED_SKILL.name}));
  ...
  eq('Grundwertverteilung exakt','ST:15|KS:16|GS:15|RF:14|FF:18|WL:17|IT:23|IN:19|WN:18|CR:17',ATTRS.map(function(attribute){return attribute[0]+':'+attributeDistributionV1715()[attribute[0]]}).join('|'));
